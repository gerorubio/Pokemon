            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        local difHP = self.playerPokemon.HP - oldHP
                        local difAttack = self.playerPokemon.attack - oldAttack
                        local difDefense = self.playerPokemon.defense - oldDefense
                        local difSpeed = self.playerPokemon.speed - oldSpeed

                        local levelUpText = {}
                        table.insert(levelUpText , 'HP: ' .. oldHP .. ' + ' .. difHP .. ' = ' .. self.playerPokemon.HP)
                        table.insert(levelUpText , 'Atk: ' .. oldAttack .. ' + ' .. difAttack .. ' = ' .. self.playerPokemon.attack)
                        table.insert(levelUpText , 'Def: ' .. oldDefense .. ' + ' .. difDefense .. ' = ' .. self.playerPokemon.defense)
                        table.insert(levelUpText , 'Spd: ' .. oldSpeed .. ' + ' .. difSpeed .. ' = ' .. self.playerPokemon.speed)
                        gStateStack:push(LevelUpMenuState(self, levelUpText))
                    else
                        self:fadeOutWhite()
                    end
                end)
